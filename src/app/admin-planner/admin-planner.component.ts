...
export class AdminPlannerComponent implements OnInit {
  ...
  constructor(private snotifyService: SnotifyService,public router:Router,public _router:ActivatedRoute,public plannerService:PlannerServiceService,public modal:NgbModal,public userService:UserService,
  public socketService:SocketService) { }

  // stores the user ID of the current user
  public currentUserId;

  public adminUserDetails = this.userService.getUserInfoFromLocalStorage();
  ...
  // code for limiting number of months
  ...
  public getAllEvents = () =>{
    ...
    this.events = [] ;
    this.plannerService.getAllEventsUsingId(this.currentUserId).subscribe((response)=>{
  ...
  public saveEvent = () =>{
    ...
    // console.log(details);
    this.plannerService.createEvent(details).subscribe((response)=>{

      if(response.status === 200){
        this.getAllEvents();
        this.click(1,1);
        this.snotifyService.success('success','Event added');
      }
  ...
  public saveEditedEvent = () =>{
    ...
    this.plannerService.updateEvent(details).subscribe((response)=>{
      if(response.status === 200){
        this.getAllEvents();
        this.snotifyService.success('success','Event edited');
        this.click(1,1);
      }
    })
  ...
  public deleteEvent = (event) =>{
    ...
    this.plannerService.deleteEvent(details).subscribe((response)=>{
      ...
      this.snotifyService.success('Success','event deleted')
    })
...
public getAllDetailsOfCurrentUser = () =>{
  this.userService.getAUser(this.currentUserId).subscribe((response)=>{
